...
export const Matchmaking: React.FC = () => {
  ...
  return (
    <div className="h-[calc(100dvh-8rem)] flex flex-col items-center justify-center relative overflow-hidden">
      ...
      {/* Card Stack */}
      <div className="relative w-full max-w-sm md:max-w-md h-[55vh] md:h-[500px]">
        ...
        >
          <div className="h-3/5 bg-cover bg-center relative" style={{ backgroundImage: `url(${currentProfile.avatar})` }}>
            ...
            <div className="absolute top-4 right-4 bg-neon-cyan text-black font-bold px-3 py-1 rounded-full flex items-center gap-1 shadow-lg">
              <Zap size={14} fill="currentColor" /> {currentProfile.matchPercentage}%
            </div>
          </div>

          <div className="p-4 md:p-6 space-y-3 md:space-y-4 h-2/5 flex flex-col">
            <p className="text-gray-300 italic text-sm md:text-base line-clamp-2">"{currentProfile.bio}"</p>
            
            <div className="flex gap-2 md:gap-4 mt-auto">
              <div className="flex-1 bg-neon-red/10 p-2 md:p-3 rounded-xl border border-neon-red/20">
                <p className="text-[10px] font-bold text-neon-red uppercase tracking-wider">Wants to Learn</p>
                <p className="font-bold text-white text-sm md:text-base">{currentProfile.wantsToLearn}</p>
              </div>
              <div className="flex-1 bg-neon-cyan/10 p-2 md:p-3 rounded-xl border border-neon-cyan/20">
                <p className="text-[10px] font-bold text-neon-cyan uppercase tracking-wider">Can Teach</p>
                <p className="font-bold text-white text-sm md:text-base">{currentProfile.canTeach}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="flex items-center gap-4 md:gap-6 mt-6 md:mt-8">
        <button 
          ...
          className="w-14 h-14 md:w-16 md:h-16 rounded-full bg-gray-800 text-neon-red border border-gray-700 hover:bg-gray-700 flex items-center justify-center transition-all hover:scale-110 shadow-lg active:scale-95"
        >
          <X size={28} />
        </button>
        <button 
          onClick={() => handleSwipe('right')}
          className="w-16 h-16 md:w-20 md:h-20 rounded-full bg-gradient-to-tr from-neon-cyan to-blue-500 text-black flex items-center justify-center transition-all hover:scale-110 hover:shadow-[0_0_25px_#00FFD1] shadow-xl active:scale-95"
        >
          <Heart size={32} fill="black" />
        </button>
        <button className="w-14 h-14 md:w-16 md:h-16 rounded-full bg-gray-800 text-white border border-gray-700 hover:bg-gray-700 flex items-center justify-center transition-all hover:scale-110 shadow-lg active:scale-95">
          <Star size={24} />
        </button>
      </div>
      ...
      {matched && (
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/90 backdrop-blur-sm animate-in fade-in duration-300 p-4">
          <div className="text-center w-full max-w-md">
            <h2 className="text-5xl md:text-6xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-neon-cyan to-neon-purple mb-2 italic transform -rotate-6">
              It's a Match!
            </h2>
            ...
            <div className="flex justify-center items-center gap-4 md:gap-8 mb-10">
              <div className="w-20 h-20 md:w-24 md:h-24 rounded-full overflow-hidden border-4 border-neon-cyan shadow-[0_0_20px_#00FFD1]">
              ...
              <div className="w-20 h-20 md:w-24 md:h-24 rounded-full overflow-hidden border-4 border-neon-purple shadow-[0_0_20px_#B026FF]">
            ...
            <div className="flex flex-col gap-3">
              ...
              <button 
                ...
                className="text-gray-400 font-bold hover:text-white transition-colors py-2"
              >
                Keep Swiping
              </button>
            </div>
